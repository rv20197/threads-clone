...
export async function fetchPosts(pageNumber = 1, pageSize = 20, doFetchParentThreads = true, currentUserId = null) {
   connectToDB();
...
export async function likeThread(threadId: string, userId: string) {
   ...
   try {
      ...
      if (originalThread.likes && originalThread.likes.length > 0) {
         const index = originalThread.likes.findIndex((id: ObjectId) => JSON.stringify(id) === JSON.stringify(user._id));
         if (index === -1) {
            originalThread.likes.push(user._id);
         } else {
            originalThread.likes = originalThread.likes.filter((id: any) => id !== user._id);
         }
      } else {
         originalThread.likes = [];
         originalThread.likes.push(user._id);
      }
